...
i18n:             next-intl
Galería/Lightbox: PhotoSwipe 5 (lightbox responsive + zoom + gestos táctiles)
...
│   │   │   ├── TourCard.tsx
│   │   │   ├── TourFilters.tsx
│   │   │   ├── TourGallery.tsx          # PhotoSwipe 5 lightbox + Cloudinary srcset
│   │   │   │                            # Grid responsive → clic abre lightbox fullscreen
│   │   │   │                            # Zoom, swipe, pinch-to-zoom, captions dinámicos
│   │   │   │                            # Carga dinámica: import() solo al abrir
...
### 6.4 Galería de Tours — PhotoSwipe 5 + Cloudinary

La galería de cada tour usa PhotoSwipe como lightbox y Cloudinary como proveedor de imágenes. Juntos crean una experiencia de galería profesional con carga progresiva.

**Arquitectura de imágenes (3 niveles):**

```
Cloudinary genera automáticamente 3 versiones de cada foto:

1. THUMBNAIL  → c_fill,w_400,h_300,q_auto,f_auto   (~20KB)  → Grid del catálogo
2. MEDIUM     → c_fill,w_800,h_600,q_auto,f_auto   (~80KB)  → Ficha del tour
3. FULL       → c_limit,w_1920,q_auto,f_auto        (~200KB) → PhotoSwipe lightbox + zoom
```

**Flujo de carga progresiva:**

```
1. Usuario ve catálogo         → Carga THUMBNAIL (20KB cada una, instantáneo)
2. Usuario abre ficha de tour  → Carga MEDIUM (80KB, rápido)
3. Usuario hace clic en foto   → PhotoSwipe se importa dinámicamente (solo aquí)
                                → Abre lightbox con imagen MEDIUM
                                → Carga FULL en background vía srcset
4. Usuario hace zoom           → PhotoSwipe muestra FULL automáticamente (srcset)
```

**Componente TourGallery.tsx (implementación):**

```typescript
// src/components/public/TourGallery.tsx
"use client";

import { useEffect, useRef } from "react";
import type { TourImage } from "@prisma/client";

// Utilidad para generar URLs de Cloudinary con transformaciones
...
export function TourGallery({ images, tourName }: TourGalleryProps) {
  ...
  useEffect(() => {
    // Importación dinámica: PhotoSwipe solo se carga cuando el componente se monta
    let lightbox: any;
  ...
  return (
    <div ref={galleryRef} className="grid grid-cols-2 md:grid-cols-3 gap-2">
      {sorted.map((img, i) => {
        const thumbUrl = cloudinaryUrl(img.cloudinaryId, "c_fill,w_800,h_600,q_auto,f_auto");
        const fullUrl = cloudinaryUrl(img.cloudinaryId, "c_limit,w_1920,q_auto,f_auto");
        return (
          <a
            key={img.id}
            href={fullUrl}
...
**Integración admin ↔ web:**

```
Admin sube foto → Cloudinary Widget → Genera 3 versiones auto
                                     → Retorna cloudinaryId + URL
                                     → Guarda en TourImage (DB)
                                     → Reordenar con drag & drop (dnd-kit)
                                     → Marcar isPrimary con estrella

Web pública     → TourGallery lee TourImage[] de DB
                → Grid con URLs MEDIUM de Cloudinary
                → PhotoSwipe abre FULL al hacer clic + zoom con srcset
```

### 6.5 Flujo: Crear tour nuevo desde admin (cómo se conecta con la web)
...
### 6.6 Flujo: Link de pago personalizado (técnico)

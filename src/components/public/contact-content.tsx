"use client";

import { useTranslations } from "next-intl";
import { Phone, Mail, ArrowUpRight } from "lucide-react";
import { useScrollAnimation } from "@/hooks/use-scroll-animation";

export function ContactContent() {
  const t = useTranslations("contact");
  const tc = useTranslations("common");
  const whatsappNumber = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "51984123456";

  const heroAnim = useScrollAnimation({ threshold: 0.1 });
  const contentAnim = useScrollAnimation({ threshold: 0.1 });

  return (
    <>
      {/* Hero header */}
      <section className="bg-[#faf8f5] border-b border-gray-100">
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-12 py-20 lg:py-28">
          <div
            ref={heroAnim.ref}
            className={`${heroAnim.isVisible ? "scroll-visible" : "scroll-hidden"} text-center`}
          >
            <p className="text-sm font-medium uppercase tracking-[0.2em] text-brand-teal mb-4">
              {t("title")}
            </p>
            <h1 className="font-heading text-4xl font-light text-brand-darkRed sm:text-5xl lg:text-[3.5rem] tracking-tight leading-[1.1]">
              {t("title").split(" ")[0]}{" "}
              <span className="font-serif italic font-normal text-brand-teal">
                {t("title").split(" ").slice(1).join(" ") || "con nosotros"}
              </span>
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-gray-500 font-light leading-relaxed">
              {t("subtitle")}
            </p>
          </div>
        </div>
      </section>

      {/* Contenido */}
      <section className="bg-white py-16 lg:py-24">
        <div
          ref={contentAnim.ref}
          className={`${contentAnim.isVisible ? "scroll-visible" : "scroll-hidden"} mx-auto max-w-3xl px-4 sm:px-6 lg:px-12`}
        >
          {/* WhatsApp principal */}
          <div className="rounded-3xl bg-brand-teal/5 border border-brand-teal/15 p-10 sm:p-14 text-center mb-10">
            <div className="flex h-16 w-16 mx-auto items-center justify-center rounded-full bg-[#25D366]/15 mb-6">
              <svg viewBox="0 0 24 24" className="h-8 w-8 fill-[#25D366]">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
            </div>
            <h2 className="font-heading text-2xl font-bold text-brand-darkRed mb-2 sm:text-3xl">
              {t("whatsapp_cta", { fallback: "¿Prefieres WhatsApp?" })}
            </h2>
            <p className="text-gray-500 mb-8 max-w-md mx-auto leading-relaxed">
              {t("reply_fast")}
            </p>
            <a
              href={`https://wa.me/${whatsappNumber.replace(/\D/g, "")}?text=${encodeURIComponent("Hola, quiero información sobre tours")}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-between rounded-full bg-[#25D366] hover:bg-[#1fb855] text-white pl-7 pr-2 py-2.5 text-base font-bold transition-all group shadow-lg hover:shadow-xl hover:scale-[1.02]"
            >
              {t("whatsapp_button", { fallback: "Escríbenos por WhatsApp" })}
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-white/20 group-hover:bg-white group-hover:text-[#25D366] transition-colors ml-4">
                <ArrowUpRight className="h-5 w-5" />
              </span>
            </a>
          </div>

          {/* Datos de contacto alternativos */}
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <div className="flex items-center gap-3 rounded-2xl bg-[#faf8f5] px-6 py-5 flex-1">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-teal/10">
                <Phone className="h-4 w-4 text-brand-teal" />
              </span>
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">
                  {tc("phone")}
                </p>
                <p className="text-sm font-medium text-gray-800 mt-0.5">+51 84 123 456</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-2xl bg-[#faf8f5] px-6 py-5 flex-1">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-teal/10">
                <Mail className="h-4 w-4 text-brand-teal" />
              </span>
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">
                  {tc("email")}
                </p>
                <p className="text-sm font-medium text-gray-800 mt-0.5">info@likeinhouse.com</p>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}

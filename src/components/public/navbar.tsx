...
export function Navbar() {
  ...
  // Hero mode: homepage + not scrolled → floating glass pills
      ...
      <header className="fixed top-0 left-0 right-0 z-50 w-full pointer-events-none py-5 lg:py-7">
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8 flex items-center justify-between relative h-20">
          ...
          <div className="pointer-events-none absolute left-0 right-0 flex justify-center top-1/2 -translate-y-1/2 z-10">
            <nav
              ref={navRef}
              className="pointer-events-auto flex items-center gap-0.5 rounded-full p-1.5 shadow-2xl border backdrop-blur-xl relative bg-black/45 border-white/10"
            >
  ...
  // ─── STANDARD MODE: white sticky bar ─────────────────────────────────────
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full bg-white/97 backdrop-blur-md shadow-sm transition-shadow duration-300">
